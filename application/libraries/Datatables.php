...
class Datatables
{
    ...
    private function produce_output($output, $charset)
    {
        ...
        if ($output == 'json') {
            $sOutput = array('draw' => intval($this->ci->input->post('draw')), 'recordsTotal' => $iTotal, 'recordsFiltered' => $iFilteredTotal, 'data' => $aaData);
            if ($charset == 'utf-8') {
        ...
        if (isset($custom_val['replacement']) && is_array($custom_val['replacement'])) {
            ...
            foreach ($custom_val['replacement'] as $key => $val) {
                ...
                if (preg_match('/(\\w+::\\w+|\\w+)\\((.*)\\)/i', $val, $matches) && is_callable($matches[1])) {
                    $func = $matches[1];
                    $args = preg_split("/[\\s,]*\\\"([^\\\"]+)\\\"[\\s,]*|[\\s,]*'([^']+)'[\\s,]*|[,]+/", $matches[2], 0, "PREG_SPLIT_NO_MM_WYPTURE");
