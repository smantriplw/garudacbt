...
class Ion_auth
{
    ...
    public function forgotten_password($identity)
    {
        ...
        $user = $this->where($this->ion_auth_model->identity_column, $identity)->where('active', 1)->users()->row();
        if ($user) {
            // Generate code
            $code = $this->ion_auth_model->forgotten_password($identity);
            if ($code) {
                $data = ['identity' => $identity, 'forgotten_password_code' => $code];
    ...
    public function register($identity, $password, $email, $additional_data = [], $group_ids = [])
    {
        ...
        if (!$email_activation) {
            ...
            $data = ['identity' => $user->{$identity}, 'id' => $user->id, 'email' => $email, 'activation' => $activation_code];
            if (!$this->config->item('use_ci_email', 'ion_auth')) {
    ...
    protected function check_compatibility()
    {
        ...
        if (!function_exists('password_hash') || !function_exists('password_verify')) {
            show_error("PHP function password_hash or password_verify not found. Are you using CI 2 and PHP < 5.5? Please upgrade to CI 3, or PHP >= 5.5 or use password_compat (https://github.com/ircmaxell/password_compat).");
        }
        ...
        if (!function_exists('random_bytes') && !function_exists('mcrypt_create_iv') && !function_exists('openssl_random_pseudo_bytes')) {
            show_error("No CSPRNG functions to generate random enough token. Please update to PHP 7 or use random_compat (https://github.com/paragonie/random_compat).");
        }
    }
    public function deactivate($id = NULL)
    {
        $this->ion_auth_model->trigger_events('deactivate');
        //$this->ion_a->trigger_events('deactivate');
        if (!isset($id)) {
            $this->set_error('deactivate_unsuccessful');
            return false;
        } else {
            if ($this->logged_in() && $this->user()->row()->id == $id) {
